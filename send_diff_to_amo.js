...
const PIPELINE_ID = 9677626;
const STATUS_ID = 77172594;
...
async function createExperimentalLead(contactId, text) {
  await axios.post(`${AMO_URL}/api/v4/leads`, [
    {
      name: text.split('\n')[0], // например: "Платёж #123"
...
(async () => {
  ...
  for (const row of rows) {
    ...
    try {
      await createExperimentalLead(contactId, text);
      logEntry.leadCreated = true;
      console.log(`✅ Сделка по платёж #${row.ID} → контакт ${contactId}`);
      successCount++;
    } catch (err) {
      logEntry.error = `Ошибка создания сделки: ${err.message}`;
      console.error(`❌ Ошибка при сделке ID ${row.ID}: ${err.message}`);
    }

    logs.push(logEntry);

    if (successCount >= 10) {
      console.log('🛑 Отправлено 10 успешных сделок. Остановлено.');
      break;
    }
  }
