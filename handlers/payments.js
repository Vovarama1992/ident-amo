...
module.exports = async function (rows) {
  ...
  for (const rowRaw of rows) {
    // нормализуем ID
    const row = { ...rowRaw };
    if (row.ID === undefined && row.ID_Orders !== undefined) {
      row.ID = row.ID_Orders;
    }
    ...
    const payload = {
      ...
      id: row.ID,
      sum: parseFloat(row.Payment || row.Amount || 0),
      date: (row.DateTimePayment || row.DateTimeBill || '').split(' ')[0]
    };
  ...
  const logName = `payments_${nowStr()}.json`;
